...
import sk.kra.files.provider.kra.KraPath
import sk.kra.files.provider.kra.KraCopyMove
...
internal object ForeignCopyMove {
    ...
    fun copy(source: Path, target: Path, vararg options: CopyOption) {
        ...
        // Special handling for KRA provider - use KraCopyMove for better progress reporting
        when {
            source is KraPath && target !is KraPath -> {
                // KRA → Foreign (e.g., KRA → Local)
                val authentication = KraFileSystemProvider.getAuthentication(source)
                val client = KraFileSystemProvider.getClient(source.fileSystem.authority, authentication)
                KraCopyMove.copyToForeign(source, target, copyOptions, client)
                return
            }
            target is KraPath && source !is KraPath -> {
                // Foreign → KRA (e.g., Local → KRA)
                val authentication = KraFileSystemProvider.getAuthentication(target)
                val client = KraFileSystemProvider.getClient(target.fileSystem.authority, authentication)
