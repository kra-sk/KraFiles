...
import sk.kra.files.coil.AppIconPackageName
import sk.kra.files.compat.foregroundCompat
import sk.kra.files.compat.getDrawableCompat
import sk.kra.files.compat.isSingleLineCompat
import sk.kra.files.databinding.FileItemGridBinding
import sk.kra.files.databinding.FileItemListBinding
import sk.kra.files.file.FileItem
import sk.kra.files.file.fileSize
import sk.kra.files.file.formatShort
import sk.kra.files.file.iconRes
import sk.kra.files.file.isApk
import sk.kra.files.provider.archive.isArchivePath
import sk.kra.files.provider.common.isEncrypted
import sk.kra.files.provider.common.UploadStateManager
import sk.kra.files.settings.Settings
import sk.kra.files.ui.AnimatedListAdapter
import sk.kra.files.ui.CheckableForegroundLinearLayout
import sk.kra.files.ui.CheckableItemBackground
import sk.kra.files.util.isMaterial3Theme
...
class FileListAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int, payloads: List<Any>) {
        ...
        // Update upload progress bar if file is being uploaded
        holder.uploadProgressBar?.let { progressBar ->
            val uploadState = UploadStateManager.getUploadState(path)
            if (uploadState != null) {
                progressBar.isVisible = true
                progressBar.max = 100
                progressBar.progress = uploadState.progressPercent
            } else {
